...
* `pols-month.csv`: This dataset contains monthly U.S. political composition. After cleaning, it contains integers for `year` and `month` (as month names), party counts (`gov_gop`, `sen_gop`, `rep_gop`, `gov_dem`, `sen_dem`, `rep_dem`), and `president` variable indicating the president’s party that month.
...
* For the available data, Professor Trash Wheel collected `r round(prof_total_weight_tons, 0)` tons in total, and Gwynnda collected `r round(gwyn_cigs_jun2022, 0)` cigarette butts in June 2022.
...
drops_tbl
```
The table shows the 10 ZIPs with the largest ZORI declines from Jan-2020 to Jan-2021, consistent with pandemic demand shifts in denser areas. 
